...
class PROVisualizer extends HTMLElement {
	...
	connectedCallback() {
		...
		/*
		provisualizerScript.substring(0, provisualizerScript.lastIndexOf('provisualizer.js'));
		if (baseUrl == "") {
		...
		*/
		...
		style.href = baseUrl + 'provisualizer.css';
...
var provisualizer = d3.select(this.shadowRoot).select("#provisualizer");
//.append("div")
...
function updateKeyItem(className, label, nodes) {
	...
	provisualizer.select(".key text." + className).text(label + " (" + filteredNodes.length + " shown)");
}
...
function createFilteredGraphFromLinks() {
	...
	var textFilter = provisualizer.select('#agency-or-function-name-filter').property("value").toUpperCase();
	var matchWholeWords = provisualizer.select('#whole-words').property("checked");
	...
	if (nodes.length == 0) {
		...
		const r = 3 * Math.sqrt((width * width) + (height * height)) / (nodes.length * nodes.length);
...
function addSearchForm() {
	...
	*/
	...
	var zoomToFitButton = buttonFieldSet.append("input")
		.attr("type", "submit")
...
function matchesDateFilter(period) {
	var yearFilterText = provisualizer.select('#year-filter').property("value");
	if (yearFilterText == "") {
...
function getSearchFragment() {
	...
	var textSearch = provisualizer.select("#agency-or-function-name-filter").property("value");
	var yearSearch = provisualizer.select("#year-filter").property("value");
	var wholeWords = provisualizer.select("#whole-words").property("checked");
